...
useEffect(()=>{
  ...
  const response=await axios.get(`https://internarea-backend-t8di.onrender.com/api/application/${id}`)
...
const handleAcceptAndReject= async(id,action)=>{
  try {
    ...
    const response=await axios.put(`https://internarea-backend-t8di.onrender.com/api/application/${id}`,{action})
